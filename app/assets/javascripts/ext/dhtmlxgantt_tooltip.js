...
gantt._tooltip = {}, gantt._tooltip_class = "gantt_tooltip", gantt.config.tooltip_timeout = 30, gantt.config.tooltip_offset_y = 20, gantt.config.tooltip_offset_x = 10, gantt._create_tooltip = function() {
    ...
    if (!gantt.config.touch || gantt.config.touch_tooltip) {
        ...
        e.x += 1 * gantt.config.tooltip_offset_x || 0, e.y += 1 * gantt.config.tooltip_offset_y || 0, e.y = Math.min(Math.max(o.y, e.y), o.y + s - i), e.x = Math.min(Math.max(o.x, e.x), o.x + r - a),
            gantt._is_cursor_under_tooltip(l, {
                pos: e,
                width: a,
                height: i
            }) && (l.x + a > r + o.x && (e.x = l.x - (a - 20) - (1 * gantt.config.tooltip_offset_x || 0)), l.y + i > s + o.y && (e.y = l.y - (i - 40) - (1 * gantt.config.tooltip_offset_y || 0))), n.style.left = e.x + "px", n.style.top = e.y + "px"
    }
    ...
    if (!this._is_tooltip(e) && (t != this._tooltip_id || this._is_task_line(e))) {
        ...
        var n = this.getTask(t),
            a = this.templates.tooltip_text(n.start_date, n.end_date, n);
